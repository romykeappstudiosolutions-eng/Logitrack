      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {conditioning.map(c => (
          <div key={c.id} onClick={() => handleOpenEdit(c)} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition-all cursor-pointer">
            <h4 className="font-black text-slate-900 mb-1">{c.documento || 'VAS-GEN'}</h4>
            ...
            <div className="flex justify-between text-xs font-bold text-slate-500">
              ...
              <span>{c.lineas} L / {c.cantidad} U</span>
            </div>
          </div>
        ))}
            ...
            <div className="p-8 space-y-6 overflow-y-auto custom-scrollbar">
              <form onSubmit={save} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1">
                    <label className="text-[8px] font-black text-slate-400 uppercase">Tipo de Proceso</label>
                ...
                <div className="space-y-1">
                  <label className="text-[8px] font-black text-slate-400 uppercase">Responsable VAS</label>
                ...
                <div className="space-y-1">
                  <label className="text-[8px] font-black text-slate-400 uppercase">Referencia / Cliente / Documento</label>
                  <input type="text" placeholder="Ej: Lote #88 / Cliente X" value={form.cliente} onChange={e=>setForm({...form, cliente:e.target.value})} className="w-full bg-slate-50 p-4 rounded-2xl text-xs font-bold outline-none" required />
                </div>
                ...
                <div className="grid grid-cols-1 gap-1">
                  <label className="text-[8px] font-black text-slate-400 uppercase">Horario de Trabajo</label>
                ...
                <div className="flex flex-col gap-3 pt-4">
                  {!isConfirming ? (
                    ...
                    <div className="bg-red-50 p-4 rounded-2xl border border-red-100 animate-in zoom-in-95">
                      <p className="text-[9px] font-black text-red-600 uppercase text-center mb-3">¿CONFIRMA QUE DESEA ELIMINAR ESTE REGISTRO VAS?</p>
                      <div className="flex gap-2">
